...
# Known vulnerable OpenSSH versions (CVE-based, legacy prefix matching)
VULNERABLE_VERSIONS = {
...
# ============================================================================
# Version Comparison Helpers (for CVE database)
# ============================================================================
...
WEAK_ALGORITHMS = {
    ...
    "cipher": {
        ...
        "3des-cbc": "Triple DES, 64-bit block size, vulnerable to Sweet32",
    ...
    "host_key": {
        ...
        "ssh-rsa": "RSA with SHA-1 signatures, deprecated in OpenSSH 8.8+",
        "ecdsa-sha2-nistp256": "NIST curve with potential backdoor concerns",
    },
}

# OS fingerprinting patterns from SSH banners (simple substring matching)
OS_FINGERPRINTS = [
...
# Enhanced regex-based fingerprinting database (for detailed analysis)
SSH_FINGERPRINTS = [
...
class ScanResult:
    ...
    def to_dict(self) -> dict:
        ...
        if d['honeypot_reasons'] is None:
            d['honeypot_reasons'] = []
        if d['fingerprint_info'] is None:
            d['fingerprint_info'] = {}
        if d['vulnerabilities'] is None:
            d['vulnerabilities'] = []
        return d
...
class SSHAuditClient:
    ...
    def _fingerprint_banner(self, banner: str) -> FingerprintInfo:
        """Analyze SSH banner to identify software and OS using regex patterns."""
    ...
    def _print_progress(self, result: ScanResult, current: int, total: int):
        ...
        if result.success:
            status = self.color.green("[SUCCESS]")
        else:
            status = self.color.red("[FAILED]")

        auth_info = f"user={result.username}"
        if result.key_file:
            auth_info += f" key={result.key_file}"

        print(
            f"\n[{current}/{total}] ({percentage:5.1f}%) "
            f"{status} "
        ...
        if result.success:
            if result.severity:
                ...
                sev_fn = sev_map.get(result.severity, self.color.dim)
                print(f"    Severity: {sev_fn(result.severity.upper())}")
            ...
            if result.host_key_changed:
                print(f"    {self.color.red('!!! HOST KEY CHANGED - possible MITM !!!')}")
                print(f"    Previous: {result.host_key_previous}")

            if result.honeypot_score >= 0.5:
                print(f"    {self.color.yellow(f'Possible honeypot (score: {result.honeypot_score:.1f})')}")
                for hr in (result.honeypot_reasons or []):
                    print(f"      - {hr}")

            if result.password_strength_label:
                strength_map = {
                    "very_weak": self.color.red,
                    "weak": self.color.red,
                    "moderate": self.color.yellow,
                    "strong": self.color.green,
                    "very_strong": self.color.green,
                }
                strength_fn = strength_map.get(result.password_strength_label, self.color.dim)
                print(
                    f"    Password Strength: "
                    f"{strength_fn(f'{result.password_strength_label.upper()} ({result.password_strength_score:.0f}/100)')}"
                )
    ...
    def scan(
        ...
        has_password_creds = usernames and (passwords or self.try_empty or self.user_as_pass)
        ...
        # Password-based work items (spray or normal mode)
        if self.spray_mode and has_password_creds:
            all_pw_sets = {}
                    ...
                    for port in ports:
                        for username in usernames:
                            ...
                            if pw_idx < len(pw_list):
                                work_items.append((host, port, username, pw_list[pw_idx], None))
        elif has_password_creds:
            for host in hosts:
                for port in ports:
                    for username in usernames:
        ...
        if has_key_creds:
            for host in hosts:
                for port in ports:
                    for username in usernames:
                        for kf in key_files:
                            work_items.append((host, port, username, "", kf))
        ...
        if self.threads > 1:
            ...
            with ThreadPoolExecutor(max_workers=self.threads) as executor:
                ...
                for h, p, u, pw, kf in work_items:
                    ...
                    future = executor.submit(self._try_login, h, p, u, pw, kf)
                    futures[future] = (h, p, u, pw, kf)
                    submit_count += 1
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        item = futures[future]
                        completed_items.append(item[:4])
                        status = self.color.green("OK") if result.success else self.color.red("FAIL")
                        if progress_bar:
            ...
            for host, port, username, password, kf in work_items:
                ...
                current += 1
                result = self._try_login(host, port, username, password, kf)
                self.results.append(result)
    ...
    def _print_summary(self):
        ...
        if self.quiet:
            ...
            if successful:
                for r in successful:
                    cred = f"{r.username}:{r.password}" if r.auth_method == "password" else f"{r.username} (key:{r.key_file})"
                    print(f"{r.host}:{r.port} {cred}")
            return
        ...
        print(f"Failed logins:         {self.color.red(str(self.stats.failed_logins))}")
        ...
        if successful:
            ...
            for r in successful:
                ...
                if r.auth_method == "key":
                    cred = f"{r.username} (key: {r.key_file})"
                else:
                    cred = f"{r.username}:{r.password}"
                print(f"  {self.color.green(f'{r.host}:{r.port}')} - {cred}{sev_tag}")
                if r.banner:
                    print(f"    {self.color.dim('Banner:')} {r.banner}")
                if r.os_info:
                    print(f"    {self.color.dim('OS:')} {r.os_info}")
                if r.host_key_type:
                    print(f"    {self.color.dim('Host Key:')} {r.host_key_type} {r.host_key_fingerprint}")
                if r.command_output:
                    output_preview = r.command_output[:200].replace('\n', ' ')
                    print(f"    {self.color.dim('Command Output:')} {output_preview}")
                elif r.initial_output:
                    output_preview = r.initial_output[:200].replace('\n', ' ')
                    print(f"    {self.color.dim('Output:')} {output_preview}...")
                if r.severity_reasons:
                    for reason in r.severity_reasons:
                        print(f"    {self.color.yellow('!')} {reason}")
                if r.host_key_changed:
                    print(f"    {self.color.red('!!! HOST KEY CHANGED !!!')}")
                if r.honeypot_score >= 0.5:
                    print(f"    {self.color.yellow(f'Possible honeypot (score: {r.honeypot_score:.1f})')}")
                ...
                if r.vulnerabilities:
                    for vuln in r.vulnerabilities:
                        sev_color = self.color.red if vuln.severity == "HIGH" else self.color.yellow
                        print(f"    {sev_color(f'[{vuln.severity}]')} {vuln.cve} - {vuln.name}")
                if r.fingerprint_info and r.fingerprint_info.software:
                    print(f"    {self.color.dim('Software:')} {r.fingerprint_info.software} {r.fingerprint_info.software_version}")
            print()

        # Vulnerability summary (deduplicated)
        all_vulns = {}
        for r in self.results:
            if r.vulnerabilities:
                for v in r.vulnerabilities:
                    all_vulns[v.cve] = v
        if all_vulns:
            print(f"\n{self.color.bold('VULNERABILITY FINDINGS:')}")
            print("-" * 40)
            for cve, v in sorted(all_vulns.items()):
                sev_color = self.color.red if v.severity == "HIGH" else self.color.yellow
                print(f"  {sev_color(f'[{v.severity}]')} {v.cve}: {v.name}")
                print(f"    {self.color.dim(v.description)}")
            print()
...
def main():
    ...
    if args.key_file:
        try:
            client = SSHAuditClient()
            key_files.extend(client._read_file_lines(args.key_file, "key"))
        except (FileNotFoundError, PermissionError, IOError) as e:
            print(str(e), file=sys.stderr)
            sys.exit(1)
    ...
    if args.combo_file:
        try:
            client = SSHAuditClient()
            combo_list = client._parse_combo_file(args.combo_file)
        except (FileNotFoundError, PermissionError, IOError) as e:
            print(str(e), file=sys.stderr)
            sys.exit(1)

    # Validate: must have at least some credentials
    has_password_creds = users and (passwords or args.try_empty or args.user_as_pass)
